...
// Define all required mutexes here			DONE
pthread_mutex_t pot_mutex = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t ingridient_mutex = PTHREAD_MUTEX_INITIALIZER;
...
void put_gloves(int apprentice_id) {
    ...
    // Implement a mutex lock mechanism for gloves here			DONE 
  	if(apprentice_id == 2) {
...
void remove_gloves(int apprentice_id) {
    // Implement a mutex unlock mechanism for gloves here			DONE
    if(apprentice_id == 2) {
...
void pick_ingredient(int apprentice_id, int* meal_index, int* ing_index) {
    ...
    pthread_mutex_lock(&ingridient_mutex);
    ...
    *ing_index = meal_ing_counter;
    ...
    pthread_mutex_unlock(&ingridient_mutex);
...
int main() {
	...
	// Glove Mutex initialize    GLOBAL
    for(size_t i = 0; i < GLOVE_COUNT; i++) 			// Glove threads have not been initialized in global space 
